...
const locales = {
  ...
  es: {
    ...
    months: ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julliet", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"],
...
// @kodev added config options not to be updatable by setOptions()
const themeColors = {
  blue: {
...
function processOptions(options, datepicker) {
  ...
  if (inOpts.orientation) {
    ...
    config.orientation = {
      // @kodev extended x and y center option
...
// @kodev added w-full to datepicker-picker
//@kodev added  justify center to datepicker-main
const pickerTemplate = (0,utils/* optimizeTemplateHTML */.zh)(`<div class="datepicker hidden">
  <div class="datepicker-picker inline-block rounded-lg bg-white dark:bg-gray-700 shadow-lg p-4 w-full">
    ...
    <div class="datepicker-main flex justify-center p-1"></div>
    <div class="datepicker-footer">
      <div class="datepicker-controls flex space-x-2 mt-2">
        <button type="button" class="%buttonClass% today-btn text-white %theme700% !bg-primary-700 dark:%theme600% dark:!bg-primary-600 hover:%theme800% hover:!bg-primary-800 dark:hover:%theme700% dark:hover:!bg-primary-700 focus:ring-4 focus:%themeRing% focus:!ring-primary-300 font-medium rounded-lg text-sm px-5 py-2 text-center w-1/2"></button>
        <button type="button" class="%buttonClass% clear-btn text-gray-900 dark:text-white bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 focus:ring-4 focus:%themeRing% focus:!ring-primary-300 font-medium rounded-lg text-sm px-5 py-2 text-center w-1/2"></button>
      </div>
    </div>
  </div>
</div>`);
...
class DaysView extends View {
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      if (current < this.minDate || current > this.maxDate || this.disabled.includes(current)) {
        classList.add('disabled', '!cursor-not-allowed', '!text-gray-300'); // @kodev added text-gray-300
      }
      ...
      if (this.selected.includes(current)) {
        classList.add('selected', this.picker.color700, '!bg-primary-700', 'text-white', `dark:${this.picker.color600}`, 'dark:!bg-primary-600', 'dark:text-white');
  ...
  refresh() {
    ...
    this.grid
      ...
      .forEach((el) => {
        el.classList.remove('range', 'range-start', 'range-end', 'selected', this.picker.color700, '!bg-primary-700', 'text-white', `dark:${this.picker.color600}`, 'dark:!bg-primary-600', 'dark:text-white', 'focused');
...
class MonthsView extends View {
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      if (
        ...
        classList.add('disabled', '!text-gray-300', '!cursor-not-allowed'); // @kodev added text-gray-300 cursor-not-allowed
      }
      if (range) {
  ...
  refresh() {
    ...
    this.grid
      .querySelectorAll('.range, .range-start, .range-end, .selected, .focused')
      .forEach((el) => {
        el.classList.remove('range', 'range-start', 'range-end', 'selected', this.picker.color700, '!bg-primary-700', `dark:${this.picker.color600}`, 'dark:!bg-primary-700', 'dark:text-white', 'text-white', 'focused');
...
function triggerDatepickerEvent(datepicker, type) {
  ...
  // @kodev testing
  // datepicker.inputField.setAttribute('value', datepicker.getDate())
...
function onClickClearBtn(datepicker) {
  // @kodev change clear btn behavior
...
class Picker {
  constructor(datepicker) {
    ...
    } = themeColors[datepicker.config.activeThemeColor]);

    const template = templates_pickerTemplate
                    .replace(/%buttonClass%/g, datepicker.config.buttonClass)
                    .replace(/%theme600%/g, this.color600)
                    .replace(/%theme700%/g, this.color700)
                    .replace(/%theme800%/g, this.color800)
                    .replace(/%themeRing%/g, this.colorRing)
                     ;
    ...
    (0,lib_event/* registerListeners */.cF)(datepicker, [
      ...
      [document.querySelector('#clear-pickup-details'), 'click', onClickClearBtn.bind(null, datepicker)],
    ]);
  ...
  place() {
    ...
    // @kodev extended orient x to accept center
    if (orientX === 'center') {
    ...
    // @kodev extended orient y to accept center
    if (orientY === 'center'){
    ...
    // @kodev add width to take full width of input
    // if (inputWidth > containerWidth) {
      this.element.style.minWidth = `${inputWidth}px`;
    // }
  }
  ...
  // @kodev added fade of the next and prev buttons
  ...
  setNextBtnDisabled(disabled) {
    ...
    this.controls.nextBtn.querySelector('svg').classList.toggle('!text-gray-300', disabled)
  }
...
class Datepicker {
  ...
  constructor(element, options = {}, rangepicker = undefined) {
    ...
    const config = this.config = Object.assign({
      ...
      activeThemeColor: options.activeThemeColor,
      container: document.body.querySelector('#page-content') ?? document.body, // @kodev testing --project specific
  ...
  enterEditMode() {
    ...
    this.editMode = true;
    this.inputField.classList.add('in-edit', this.colorBorder, '!border-primary-700');
  }
  ...
  exitEditMode(options = undefined) {
    ...
    this.inputField.classList.remove('in-edit', this.colorBorder, '!border-primary-700');
    if (opts.update) {
      this.update(opts);
    }
  }
...
//  @kodev extended functionality for min and max date colors
  ...
  // @kodev extended
  if (activeColor) {
    options.activeThemeColor = activeColor;
...
if (maxDate) {
  ...
  if (lang) {
    console.log(lang, flowbite_datepicker_Datepicker__WEBPACK_IMPORTED_MODULE_0__/* .default.locales */ .Z.locales);
    if (lang in flowbite_datepicker_Datepicker__WEBPACK_IMPORTED_MODULE_0__/* .default.locales */ .Z.locales) {
