...
from admin_bp.dashboard.core.config.parser import (
    parse_string,
    parse_version,
    parse_bool,
    parse_literal,
...
class Config(object):
    ...
    def __init__(self):
        ...
        # dashboard
        self.version = '1.0'
        self.blueprint_name = 'inter.admin.dashboard'
        self.link = 'dashboard'
    ...
    def init_from(self, file=None, envvar=None, log_verbose=False):
        ...
        try:
            ...
            # database @kodev update
            # self.database_name = parse_string(parser, 'database', 'DATABASE', self.database_name)
            # db_name = parse_string(parser, 'database', 'DATABASE', self.database_name)
            # if db_name.startswith('sqlite:///'):
            #     self.database_name = db_name
            # else:
            #     database_path ='sqlite:///' + os.path.join(PROJECT_PATH,'instance', db_name)
            #     self.database_name = database_path
            self.database_name = os.getenv('DB_URL')
